...
# 플랫폼별 크롤링한 리뷰 넣기
def insert_crawled_reviews(website):
    with app.app_context():
...
# 음식점과 위치, 업종, 데이터 넣기
def insert_res_cats_menus(res_loc_cat_connected):
    with app.app_context():
...
# 플랫폼별 총 평점 넣기
def insert_total_rating():
    with app.app_context():
        with open(f"./dataset/total_rating_platform.csv", "r", encoding="utf8") as data:
            reader = csv.DictReader(data)
            for lines in reader:
                name = lines["name"]
